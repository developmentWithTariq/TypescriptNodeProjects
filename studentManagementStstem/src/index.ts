...
import { ShowStatus } from "./types.js";
...
function corsePayment(course: string): void {
    prompt({ 
    type: "list", 
    name: "selectCourse", 
    ...
    choices: [course]
            ...
            let stdInfo: Object = system.getStudents();
            let result: boolean = system.payFee(stdInfo[0].id,[answer["selectCourse"]]);
...
    prompt({
    ...
    choices: ["AI","IOT","CNC"]
        ...
        let stdInfo: Object = system.getStudents()
        let result: boolean = system.enroll(stdInfo[0].id,answer["selectCourse"])
        if(result){
...
    prompt({
    ...
    choices: ["ENROLL","PAY FEE","VIEW BALANCE","SHOW STATUS","OPEN PORTAL","LOG OUT"]
            ...
            // console.log(action)
            let student: Object = system.getStudents();
    ...
    prompt({ 
        ...
        .then(answer => {
            ...
            if(system.isEnrolled(student[0].id,answer["selectCourse"])){
                ...
                let status:ShowStatus = system.checkStatus(student[0].id);
...
function start(): void{
    prompt({
        ...
        if(answer["name"] != ""){
            ...
            let std:Object = system.getStudents();
            console.log(`\n\t-----------------------------------------------`);

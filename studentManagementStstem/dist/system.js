import { Courses } from "./types.js";
export class StudentsManagmentSystem {
    constructor() {
        this.studentList = [];
        this.enrollCourses = new Map().set(0, {
            AI: false,
            CNC: false,
            IOT: false
        });
        this.status = new Map();
        this.studentDetails = new Map().set(0, {
            studentName: "",
    ...
    enroll(id, courseName) {
        let isStd = this.isStudent(id);
        if (isStd) {
            let isEnrolled = this.isEnrolled(id, courseName);
            if (isEnrolled) {
                return false;
            }
            else {
    ...
    payFee(id, courseName) {
        let studentName = this.studentList.find(std => std.id === id);
        for (let i = 0; i < courseName.length; i++) {
            let isEnrolled = this.isEnrolled(id, courseName[i]);
            if (isEnrolled) {
                this.status.get(id).studentBalance -= 200;
                this.studentDetails.set(id, {
                    studentName: studentName.name,
    ...
    isEnrolled(id, courseName) {
        let idStd = this.isStudent(id);
        if (idStd) {
            let eCourses = this.enrollCourses.get(id);
            switch (courseName) {
                case Courses.course1:
                    return eCourses.AI;
                case Courses.course2:
    ...
    isStudent(id) {
        let isSTd = this.studentList.find(std => std.id === id);
        return !!isSTd;
    }
}
// type Course ={
